...
```bash
mkdir -p 'terraform/modules/elasticache-serverless-redis-and-memcached'
gh -R 'pbs/terraform-aws-elasticache-serverless-redis-and-memcached-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/elasticache-serverless-redis-and-memcached'
rm -f release.tar.gz
```
...
```bash
git remote add -f elasticache-serverless-redis-and-memcached git@github.com:pbs/terraform-aws-elasticache-serverless-redis-and-memcached-module.git
git subtree add --prefix terraform/modules/elasticache-serverless-redis-and-memcached elasticache-serverless-redis-and-memcached main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/elasticache-serverless-redis-and-memcached elasticache-serverless-redis-and-memcached $REF --squash
```
